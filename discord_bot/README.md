...
(`/school learn`) feed into `/attack`. As with Kata, the bot auto-applies the
verbatim-verified subset whose condition it can actually evaluate (stance, weapon,
or always-on); everything else stays DM-adjudicated (full text on the sheet via
`/school view`). Tranche 1:

| Technique | Auto-applied |
|---|---|
