...
**Raises and Emphases (GDD s41 / s24, enforced):** called Raises on an attack (including a maneuver's cost after Free Raises) or a spell may not exceed the caster's **Void Ring**; an **Unskilled** attack may not use Raises of any kind (called, maneuver or Free). Every `/check` command takes an optional `emphasis` that must be on the sheet for that Skill and rerolls 1s once; `/fight attack` applies a weapon Emphasis (e.g. Kenjutsu: Katana) automatically, as does `/assess investigate` with its emphasis choice.
