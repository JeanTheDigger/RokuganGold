...
func _npc_investigate(es: EntityState) -> Dictionary:
	# Move toward noise source at investigation-style speed.
	var moved: bool = false
	if es.noise_src_x >= 0 and es.noise_src_y >= 0:
		var budget: int = _investigation_move_budget(es)
	...
	if es.phase_rounds_left <= 0:
		if es.phase_rounds_left > -SUSPICIOUS_RETURN_ROUNDS:
			# Start returning to patrol post.
			_npc_move_toward(es, es.patrol_x, es.patrol_y, _investigation_move_budget(es))
		else:
			# Return phase done → back to UNAWARE.
