...
const SPELL_COMBAT_EFFECTS: Dictionary = {
	...
	"ravenous_swarms": {"kind": "damage", "dr_rolled": 5, "dr_kept": 3,
		"range_tiles": 6, "aoe_radius": 0},  # Fire 3: 5k3 bolt, 30'; the swarm encircles the target 5 Rounds — a Fire cast in that window auto-fails (loses the slot) + 3k3 (execute_cast_spell)
	...
	"follow_the_flame": {"kind": "damage", "dr_rolled": 6, "dr_kept": 5,
		"range_tiles": 60, "aoe_radius": 0, "fire_dot_fraction": 0.5, "fire_dot_rounds": 5},  # Fire 5: 6k5 fire stream + catches fire for half/round, 5 Rounds (GDD s35 l317; per-round re-designation not modeled)
	...
	"light_of_the_sun": {"kind": "damage_zone", "dr_rolled": 2, "dr_kept": 2, "range_tiles": 20,
		"aoe_radius": 6, "aoe_hits": "all", "duration_rounds": 10, "judgment": true},  # Fire 5: 2k2/round in 30' + 2k1 per Honor below 4, +2k2 if Tainted, Blind at Honor 0 (GDD s35 l323)
	...
	"balance_of_elements": {"kind": "heal", "heal": "dice", "heal_rolled": 3, "heal_kept": 3,
		"range_tiles": 1, "suppress_all_disadvantages": true, "suppress_rounds": 5},  # Void 4 (Ishiken): heal 3k3 + negate ALL the target's Disadvantages for 5 Rounds (ML<=3 spell-effect negation deferred — no ML on timed modifiers) (GDD s37 l155)
	...
	"the_wolfs_mercy": {"kind": "ring_change", "target": "enemy", "ring": Enums.Ring.EARTH, "delta": -1, "taint_delta": -2, "extra_damage_rolled": -1, "range_tiles": 10, "duration_rounds": 10},  # Earth 3: target's Earth Ring -1 Rank (-2 if Tainted, min 1) — reduced Wound capacity can immediately kill — AND Strength -1 (-1 rolled damage die)
