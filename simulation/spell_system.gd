...
const SPELL_COMBAT_EFFECTS: Dictionary = {
	...
	"sharing_the_strength_of_many": {"kind": "share_strength", "aoe_radius": 4, "duration_rounds": 5, "max_targets": 6},  # Earth 3: up to 6 nearest same-faction allies (incl. the caster) gain +lowest-Earth-Rank-among-them to all rolls for 5 Rounds (GDD s34 l193). Combat slice = attack rolls; does not apply to Spell Casting.
	...
	"surging_soul": {"kind": "buff", "target": "ally", "range_tiles": 2, "duration_rounds": 3,
		"mods": [{"kind": "spell_attack_rolled", "value": 1}, {"kind": "spell_attack_kept", "value": 1}, {"kind": "block_center", "value": 1}]},  # Water 2: +1k1 Attack rolls; the buffed character cannot enter/benefit from the Center Stance (execute_stance_change blocks it). The "must take a Move Action every Round" downside has no GDD-specified consequence and is not enforced (no invention).
	...
	"draw_back_the_shadow": {"kind": "dispel", "range_tiles": 20, "aoe_radius": 6},  # Air 5: within a 30' radius, dispel illusions — ML<=4 illusions (Gift of Wind, The Eye Shall Not See, Summon Fog) auto-dispel; ML5-6 illusions (Legion of the Moon) require a Contested Air Roll vs the creator. Ongoing NON-illusion spell buff/debuff effects are ALSO dispelled (owner scope A: indiscriminate, friend+foe), each via a Contested Air vs the effect's creator's element-Ring (GDD s33 l365). Untracked effects (Universal-element/kata/kiho) and Trait/Ring-delta reductions (use Balance of Elements) are not touched.
	...
	"ebbing_strength": {"kind": "trait_transfer", "target": "ally", "trait": "strength", "amount": "water_school_rank", "range_tiles": 4, "duration_rounds": 3},  # Water 1: transfer up to (Water School Rank) Strength caster->target via the per-Trait layer — the caster LOSES the Strength (lower damage/effective Water) and the ally GAINS it (higher damage/effective Water), GDD-capped at double the target's base. Strength = combat default Physical Trait (PC choice deferred to cast UI); caster-unconscious-at-0 nuance moot (trait floors at 1). GDD s36 l21.
