...
static func generate_province(
	...
	var rice_per_season: float = float(total_pu) * 0.25
	p.rice_stockpile = rice_per_season * 2.0
	p.koku_stockpile = float(p.town_pu) * 0.5
	p.iron_stockpile = float(p.mining_pu) * 0.5
