...
# -- Deprivation Effect Tests ----------------------------------------------------

func test_rice_deprivation_tick1_no_effect() -> void:
	var r: Dictionary = SupplyTetherSystem.compute_rice_deprivation_effects(1)
	assert_eq(r["morale_loss"], 0)
	assert_eq(r["health_loss"], 0)


func test_rice_deprivation_tick2_morale_only() -> void:
	var r: Dictionary = SupplyTetherSystem.compute_rice_deprivation_effects(2)
	assert_eq(r["morale_loss"], -3)
	assert_eq(r["health_loss"], 0)


func test_rice_deprivation_tick3_morale_and_health() -> void:
	var r: Dictionary = SupplyTetherSystem.compute_rice_deprivation_effects(3)
	assert_eq(r["morale_loss"], -3)
	assert_eq(r["health_loss"], -5)


func test_rice_deprivation_tick4_severe() -> void:
	var r: Dictionary = SupplyTetherSystem.compute_rice_deprivation_effects(4)
	assert_eq(r["morale_loss"], -5)
	assert_eq(r["health_loss"], -10)


func test_rice_deprivation_tick5_clamps_to_tick4() -> void:
	var r: Dictionary = SupplyTetherSystem.compute_rice_deprivation_effects(5)
	assert_eq(r["morale_loss"], -5)
	assert_eq(r["health_loss"], -10)


func test_arms_deprivation_tick1_no_effect() -> void:
	var r: Dictionary = SupplyTetherSystem.compute_arms_deprivation_effects(1)
	assert_eq(r["attack_malus"], 0)
	assert_eq(r["defense_malus"], 0)


func test_arms_deprivation_tick2_moderate() -> void:
	var r: Dictionary = SupplyTetherSystem.compute_arms_deprivation_effects(2)
	assert_eq(r["attack_malus"], -2)
	assert_eq(r["defense_malus"], -2)


func test_arms_deprivation_tick3_bad() -> void:
	var r: Dictionary = SupplyTetherSystem.compute_arms_deprivation_effects(3)
	assert_eq(r["attack_malus"], -4)
	assert_eq(r["defense_malus"], -4)


func test_arms_deprivation_tick4_severe() -> void:
	var r: Dictionary = SupplyTetherSystem.compute_arms_deprivation_effects(4)
	assert_eq(r["attack_malus"], -6)
	assert_eq(r["defense_malus"], -6)


func test_arms_deprivation_clamped_above_4() -> void:
	var r: Dictionary = SupplyTetherSystem.compute_arms_deprivation_effects(10)
	assert_eq(r["attack_malus"], -6)
	assert_eq(r["defense_malus"], -6)


func test_stationary_recovery_active_when_deprived() -> void:
	var r: Dictionary = SupplyTetherSystem.compute_stationary_recovery(2, 3)
	assert_eq(r["health_gain"], 5)
	assert_eq(r["morale_gain"], 3)
	assert_true(r["arms_recovers"])


func test_stationary_recovery_no_rice_no_morale_health() -> void:
	var r: Dictionary = SupplyTetherSystem.compute_stationary_recovery(0, 2)
	assert_eq(r["health_gain"], 0)
	assert_eq(r["morale_gain"], 0)
	assert_true(r["arms_recovers"])


func test_stationary_recovery_no_arms_no_arms_recovery() -> void:
	var r: Dictionary = SupplyTetherSystem.compute_stationary_recovery(2, 0)
	assert_eq(r["health_gain"], 5)
	assert_eq(r["morale_gain"], 3)
	assert_false(r["arms_recovers"])


func test_stationary_recovery_neither_deprived() -> void:
	var r: Dictionary = SupplyTetherSystem.compute_stationary_recovery(0, 0)
	assert_eq(r["health_gain"], 0)
	assert_eq(r["morale_gain"], 0)
	assert_false(r["arms_recovers"])
