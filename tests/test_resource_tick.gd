...
func before_each() -> void:
	...
	_province.rice_stockpile = 5.0
	_province.koku_stockpile = 2.0
	_province.iron_stockpile = 1.0


# -- Rice Consumption ----------------------------------------------------------

func test_consume_rice_civilian_cost() -> void:
	var result: Dictionary = ResourceTick.consume_rice_province(_province)
...
func test_consume_rice_stockpile_decreases() -> void:
	ResourceTick.consume_rice_province(_province)
	# 5.0 - 2.10 = 2.90
	assert_almost_eq(_province.rice_stockpile, 2.90, 0.01)
...
func test_consume_rice_deficit_when_insufficient() -> void:
	_province.rice_stockpile = 1.0
	var result: Dictionary = ResourceTick.consume_rice_province(_province)
	# cost 2.10, stockpile 1.0 -> deficit 1.10
	assert_true(result["deficit"] > 1.0)
	assert_eq(_province.rice_stockpile, 0.0)
...
func test_harvest_adds_to_stockpile() -> void:
	var old_stock: float = _province.rice_stockpile
	var meta: Dictionary = {1: {"locked_farming_pu": 4}}
	var provinces: Array[ProvinceData] = [_province]
	ResourceTick._process_harvest(provinces, meta)
	assert_almost_eq(_province.rice_stockpile, old_stock + 6.0, 0.01)
...
func test_starvation_clear_when_no_deficit() -> void:
	var result: Dictionary = ResourceTick.check_starvation(_province, 0.0, 0)
	assert_eq(result["stage"], ResourceTick.StarvationStage.CLEAR)


func test_starvation_shortage_first_season() -> void:
	var result: Dictionary = ResourceTick.check_starvation(_province, 1.0, 1)
...
func test_starvation_hunger_second_season() -> void:
	var result: Dictionary = ResourceTick.check_starvation(_province, 1.0, 2)
...
func test_starvation_famine_third_season() -> void:
	var result: Dictionary = ResourceTick.check_starvation(_province, 1.0, 3)
...
func test_iron_adds_to_stockpile() -> void:
	ResourceTick.produce_iron_province(_province, 1.0)
	assert_almost_eq(_province.iron_stockpile, 1.50, 0.01)


# -- Koku Generation -----------------------------------------------------------

func test_koku_generation_baseline() -> void:
	var result: Dictionary = ResourceTick.generate_koku_province(_province, 1.0)
	# 2 town PU × 0.25 × 1.0 = 0.50
	assert_almost_eq(result["koku_generated"], 0.50, 0.01)


func test_koku_generation_port_city() -> void:
	var result: Dictionary = ResourceTick.generate_koku_province(_province, 1.5)
	# 2 × 0.25 × 1.5 = 0.75
	assert_almost_eq(result["koku_generated"], 0.75, 0.01)


func test_koku_adds_to_stockpile() -> void:
	ResourceTick.generate_koku_province(_province, 1.0)
	assert_almost_eq(_province.koku_stockpile, 2.50, 0.01)
...
func test_growth_rate_baseline() -> void:
	var rate: float = ResourceTick.compute_growth_rate(
		_province, ResourceTick.StarvationStage.CLEAR, 0
	)
	# 5% annual / 4 = 1.25%
...
func test_growth_rate_with_peace_bonus() -> void:
	var rate: float = ResourceTick.compute_growth_rate(
		_province, ResourceTick.StarvationStage.CLEAR, 4
	)
	# (5% + 3%) / 4 = 2%
...
func test_growth_rate_strong_stockpile() -> void:
	_province.rice_stockpile = 20.0  # 20/8 = 2.5 per PU > 2.0 threshold
	var rate: float = ResourceTick.compute_growth_rate(
		_province, ResourceTick.StarvationStage.CLEAR, 0
	)
	# 10% annual / 4 = 2.5%
...
func test_growth_rate_zero_during_starvation() -> void:
	var rate: float = ResourceTick.compute_growth_rate(
		_province, ResourceTick.StarvationStage.SHORTAGE, 0
	)
	assert_eq(rate, 0.0)


func test_growth_max_rate() -> void:
	_province.rice_stockpile = 20.0
	var rate: float = ResourceTick.compute_growth_rate(
		_province, ResourceTick.StarvationStage.CLEAR, 5
	)
	# (10% + 3%) / 4 = 3.25%
