...
func test_build_context_skips_dead_siblings_in_family_bonds() -> void:
	var actor: L5RCharacterData = _make(3)
	var sibling: L5RCharacterData = _make(4)
	actor.sibling_ids = [4]
	sibling.sibling_ids = [3]
...
func test_compute_all_family_bonds_skips_dead_half_siblings() -> void:
	var actor: L5RCharacterData = _make(3)
	actor.mother_id = 2
...
func test_compute_all_family_bonds_skips_dead_cross_clan_relatives() -> void:
	var actor: L5RCharacterData = _make(3)
	var spouse: L5RCharacterData = _make(60, "Crane")
	actor.spouse_id = 60
	spouse.spouse_id = 3
