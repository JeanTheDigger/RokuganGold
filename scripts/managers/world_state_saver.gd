...
func load_world(ws: Node) -> bool:
	...
	ws.characters = _load_resource_array(base + DIR_CHARACTERS)
	ws.rebuild_characters_by_id()
	ws.active_topics = _load_resource_array(base + DIR_TOPICS)
	ws.commitments = _load_resource_array(base + DIR_COMMITMENTS)
	ws.crime_records = _load_resource_array(base + DIR_CRIMES)
	ws.insurgencies = _load_resource_array(base + DIR_INSURGENCIES)
	ws.active_courts = _load_resource_array(base + DIR_COURTS)
	ws.active_edicts = _load_resource_array(base + DIR_EDICTS)
	ws.active_hordes = _load_resource_array(base + DIR_HORDES)
	ws.ships = _load_resource_array(base + DIR_SHIPS)
	ws.children = _load_resource_array(base + DIR_CHILDREN)
	ws.constructions = _load_resource_array(base + DIR_CONSTRUCTIONS)
	ws.court_commitments = _load_resource_array(base + DIR_COURT_COMMITMENTS)
	ws.active_successions = _load_resource_array(base + DIR_SUCCESSIONS)
	...
	ws.trade_routes = _load_resource_array(base + DIR_TRADE_ROUTES)
...
func _load_json_state(ws: Node, base: String) -> void:
	...
	ws.entanglements = state.get("entanglements", [])
	ws.bound_states = state.get("bound_states", [])
	ws.active_armies = state.get("active_armies", [])
	ws.active_sieges = state.get("active_sieges", [])
	ws.active_tethers = state.get("active_tethers", [])
	ws.order_states = state.get("order_states", [])
	ws.military_companies = state.get("military_companies", [])
	ws.marriages = state.get("marriages", [])
	ws.active_assassination_ops = state.get("active_assassination_ops", [])
	ws.active_civil_wars = state.get("active_civil_wars", [])
	ws.precedent_modifiers = state.get("precedent_modifiers", {})
	ws.approach_penalties = state.get("approach_penalties", [])
	ws.active_hunts = state.get("active_hunts", [])
	ws.active_hostages = state.get("active_hostages", [])
